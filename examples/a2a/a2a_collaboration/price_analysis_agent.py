...
analysis_agent = Agent(
    ...
    retries=2,
    system_prompt=(
        "You are a cryptocurrency price analysis assistant. When a user provides a token symbol and price, "
        "use the analyze_price tool to check if the price is up or down from 24h, 7d, and 30d ago. "
...
async def analyze_price(
    ...
    # Calculate changes
    change_24h = calculate_change_percentage(current_price, historical.get("24h_ago", current_price))
    change_7d = calculate_change_percentage(current_price, historical.get("7d_ago", current_price))
    change_30d = calculate_change_percentage(current_price, historical.get("30d_ago", current_price))
    
    # Format changes with + or - signs
    def format_change(change: float):
        if change > 0:
            return f"+{change:.2f}%"
        else:
            return f"{change:.2f}%"
    
    return f"{symbol} at ${current_price:.4f} is {format_change(change_24h)} from 24h ago, {format_change(change_7d)} from 7d ago, and {format_change(change_30d)} from 30d ago."
...
async def handle_send_message(request_body: SendMessageRequest):
    ...
    try:
        ...
        error_message = f"Error analyzing price: {str(e)}"
        return SendMessageResponse(
            result=Message(
                messageId=uuid4().hex,
                role="agent",
...
if __name__ == "__main__":
    ...
    card = AgentCard(
        ...
        version="0.1.0",
        url=base_url,
        description="A simple agent that analyzes cryptocurrency prices against historical data",
        provider=AgentProvider(
            organization="Flare Foundation",
        ...
        skills=[
            AgentSkill(
                ...
                description="Analyze current cryptocurrency prices against historical data",
