...
from flare_ai_kit.agent import AgentResponse, AgentSettings, GeminiAgent
...
class FlareBlockchainAgent(GeminiAgent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.add_custom_data(
            "supported_networks", ["flare", "songbird", "coston", "coston2"]
        )
    ...
    async def analyze_blockchain_data(
        self, data_type: str, query: str
    ) -> AgentResponse:
        """
...
async def demonstrate_specialized_agents():
    ...
    code_agent = CodeReviewAgent(
        agent_id="code-reviewer-001", agent_name="Code Review Expert", settings=settings
    )
...
"""
        ...
        print(
            f"🔒 Flare-specific Security Review:\n{security_review.content[:300]}...\n"
        )
        ...
        for agent, name in [
            (flare_agent, "Flare Expert"),
            (code_agent, "Code Reviewer"),
        ]:
...
async def demonstrate_agent_persistence():
    ...
    for i, topic in enumerate(conversation_topics, 1):
        ...
        if i % 3 == 0:
            ...
            print(
                f"   📚 History length: {len(history)} (max: {agent.context.max_history_length})"
            )
    ...
    print(
        f"   Agent remembers: {agent.context.max_history_length} most recent messages"
    )
    ...
    context_test = await agent.process_input(
        "What was the main topic we were discussing?"
    )
