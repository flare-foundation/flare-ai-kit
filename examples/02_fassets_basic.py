    ...
    try:
        ...
        if "FXRP" in supported_fassets:
            ...
            try:
                ...
                print(
                    f"  Minting Vault CR: {settings.get('minting_vault_collateral_ratio')}"
                )
                print()
                ...
                if fassets.address:
                    try:
                        # Check FXRP balance
                        balance = await fassets.get_fasset_balance(
                            FAssetType.FXRP, fassets.address
                        )
                        ...
                        if fassets.sparkdex_router:
                            ...
                            print(
                                "SparkDEX router not configured - skipping allowance check"
                            )
                    except Exception as e:
                        print(
                            f"Balance/allowance check failed (expected with placeholders): {e}"
                        )
                else:
                ...
                try:
                    ...
                    if len(supported_fassets) > 1:
                        other_fassets = [
                            k for k in supported_fassets if k != "FXRP"
                        ]
                        if other_fassets:
                    ...
                    print(
                        f"Swap operations failed (expected with placeholder addresses): {e}"
                    )
                print()
                ...
                try:
                    ...
                    if agents:
                        ...
                        # Get available lots
                        available_lots = await fassets.get_available_lots(
                    ...
                    print(
                        f"Minting workflow failed (expected with placeholder addresses): {e}"
                    )
                print()
                ...
                try:
                    ...
                    if redemption_id > 0:
                        redemption_details = await fassets.get_redemption_request(
                            FAssetType.FXRP, redemption_id
                        )
                        ...
                        print(
                            f"  Payment Address: {redemption_details.get('payment_address')}"
                        )

                except Exception as e:
                    print(
                        f"Redemption operations failed (expected with placeholder addresses): {e}"
                    )
                print()

            except Exception as e:
                print(
                    f"Error with FXRP operations (expected with placeholder addresses): {e}"
                )
        ...
        if "FBTC" in supported_fassets:
            ...
            try:
                ...
                print(
                    f"Status: {'Coming Soon' if not fbtc_info.is_active else 'Active'}"
                )
            except Exception as e:
        ...
        if "FDOGE" in supported_fassets:
            ...
            try:
                ...
                print(
                    f"Status: {'Coming Soon' if not fdoge_info.is_active else 'Active'}"
                )
            except Exception as e:
