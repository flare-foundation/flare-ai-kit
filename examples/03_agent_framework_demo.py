...
from flare_ai_kit.agent import AgentSettings, GeminiAgent
...
async def main():
    ...
    try:
        ...
        if response1.usage_info:
            print(
                f"   (Tokens used: {response1.usage_info.get('total_tokens', 'N/A')})"
            )

        # Second interaction (with conversation history)
        print("\nUser: Can you help me write a Python function?")
        response2 = await agent.process_input(
        ...
        response3 = await agent.process_input(
            "I need a function that calculates the factorial of a number."
        )
        ...
        agent.add_custom_data(
            "user_preferences", {"language": "Python", "style": "functional"}
        )
        ...
        try:
            embeddings = await agent.generate_embedding(
        ...
        agent.set_system_prompt(
            "You are now a creative writing assistant specializing in science fiction."
        )
