...
class PriceAnalysisAgent:
    ...
    def __init__(self):
        ...
        # Add analysis tool
        @self.ai_agent.tool
        async def analyze_price(ctx: RunContext[AgentDependencies], symbol: str, current_price: float) -> str:
    ...
    async def handle_complex_query(self, query: str) -> str:
        ...
        try:
            ...
            if price_match:
                ...
                analysis_query = f"Analyze the current price of {symbol} at ${current_price}"
                analysis_response = await self.analysis_agent.handle_message(analysis_query)
