...
class AgentContext(BaseModel):
    ...
    custom_data: Dict[str, Any] = Field(default_factory=dict, description="Custom data for the agent")
...
class AgentResponse(BaseModel):
    ...
    usage_info: Optional[Dict[str, Any]] = Field(default=None, description="Token usage and other metrics")
...
class BaseAgent(ABC):
    ...
    async def process_input(
        ...
        **kwargs
        ...
        try:
            ...
            self.update_context(updated_at=datetime.now(timezone.utc))
            
            self.logger.info("Successfully processed input", response_length=len(response.content))
            return response
    ...
    def update_context(self, **updates) -> None:
        ...
        context_dict["updated_at"] = datetime.now(timezone.utc)
