...
class GeminiAgent(BaseAgent):
        ...
        **kwargs
        ...
        try:
            ...
            self.logger.debug(
                ...
                chunks_sent=len(content) // chunk_size + (1 if len(content) % chunk_size else 0)
            )
    ...
    async def test_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self._pydantic_agent:
                raise AgentError("PydanticAI agent not initialized")
                
            result = await self._pydantic_agent.run(test_prompt)
